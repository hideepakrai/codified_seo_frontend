...
//   handleNextPage,
//   handlePrevPage,
// } from "../redux/slices/singleIssueSlice";
...
//   const { loading, error, data, currentPage } = useSelector(
//     (state) => state.singleissue
//   );
...
//   useEffect(() => {
//     dispatch(fetchSingleIssue({ pid, rid, eid, selectedOption, currentPage }));
...
export default function IssueDetails() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6 text-gray-200">
      <div className="max-w-6xl mx-auto space-y-10">
        {/* Errors - Now at the top with back button */}
        <div className="bg-gradient-to-br from-red-900/40 to-red-800/30 backdrop-blur-xl rounded-2xl border border-red-500/30 shadow-lg p-6">
          <div className="flex justify-between items-center mb-4">
            <div className="flex gap-2 items-center">
              ...
              >
                ...
                {show == "errors" ? (
                  <ChevronUp className="w-5 h-5 text-red-300 ml-2" />
                ) : (
                  <ChevronDown className="w-5 h-5 text-red-300 ml-2" />
                )}
              </button>
            </div>
          </div>
              ...
              {errors.length === 0 ? (
                ...
                <ul className="space-y-4">
                  {errors.map((err) => (
                    <li
                      key={err.Id}
                      className="p-4 rounded-xl bg-red-900/40 border border-red-600/40 flex sm:flex-row flex-col gap-2 justify-between items-center hover:border-red-400/60 transition"
                    >
                      <div className="">
                        <p className="font-semibold text-wrap text-white flex items-center gap-2">
                          <XCircle className="w-4 h-4 text-red-300" /> {err}
                        </p>
                        <p className="text-sm text-gray-300 mt-1">
                          {issueDescriptions[err] ||
                      ...
                      <Link
                        className="px-4 py-2 rounded-lg bg-white text-black hover:bg-black hover:text-white transition font-medium w-full sm:w-48 text-center"
                        to={`/issues/view?pid=${pid}&eid=${err}`}
                      >
                        Checkout Error
                      </Link>
